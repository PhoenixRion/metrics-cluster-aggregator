 ...
 * and forwards included metrics to the wrapped sink. Excludes metrics and conditions missing a host.
 *
 * This class is thread safe.
 *
 * @author Matthew Hayter (mhayter at groupon dot com)
 */
public class HostNameFilteringSink extends BaseSink {
    ...
    public void recordAggregateData(final PeriodicData periodicData) {
        ...
        for (final AggregatedData datum : periodicData.getData()) {
            ...
            if (host == null) {
                // Filter out metrics with missing host.
                continue;
            }
            final Boolean cachedResult = _cachedFilterResult.getUnchecked(host);
            if (cachedResult) {
        ...
        for (final Condition condition : periodicData.getConditions()) {
            final String host = periodicData.getDimensions().get("host");
            if (host == null) {
                // Filter out conditions with missing host.
                continue;
            }
            final Boolean cachedResult = _cachedFilterResult.getUnchecked(host);
            if (cachedResult) {
     ...
     */
    protected HostNameFilteringSink(final Builder builder) {
        super(builder);
     ...
     */
    public static final class Builder extends BaseSink.Builder<Builder, HostNameFilteringSink> {

        /**
         * Public constructor.
         */
        public Builder() {
            super(HostNameFilteringSink::new);
        }

        /**
         * Sets exclude filters. Exclude filters are regular expressions matched
         * against host names. Include filters take precedence over exclude
